...
pub fn run_post_edit(tenant: Option<&str>) -> anyhow::Result<()> {
    ...
    if let Some(text) = credential_output_advisory(&buf) {
        sections.push(text);
    }
...
fn credential_output_advisory(input_json: &str) -> Option<String> {
...
mod tests {
    ...
    fn credential_payload(session: &str, output: &str) -> String {
        serde_json::json!({
            "session_id": session,
            "tool_name": "Bash",
    ...
    #[test]
    fn credential_output_advises_once_for_an_unchanged_cause() {
        ...
        let first = credential_output_advisory(&payload).expect("first cause must advise");
        assert!(first.contains("bearer"));
        assert!(
            !first.contains("synthetic_token"),
            "must never echo the match"
        );
        assert_eq!(credential_output_advisory(&payload), None);
    }

    #[test]
    fn credential_output_re_advises_when_the_stable_cause_changes() {
        let session = format!("credential-output-change-{}", std::process::id());
        let bearer = credential_payload(
            &session,
            "Authorization: Bearer synthetic_token_0123456789abcdef",
        );
        let github = credential_payload(&session, "ghp_0123456789abcdefghijklmnop");
        assert!(credential_output_advisory(&bearer).is_some());
        let changed = credential_output_advisory(&github).expect("new class must advise again");
        assert!(changed.contains("github"));
        assert!(!changed.contains("ghp_"), "must never echo the match");
    }

    #[test]
    fn ordinary_output_and_malformed_payload_stay_silent() {
        assert_eq!(credential_output_advisory("not json"), None);
        let payload = credential_payload("credential-output-clean", "build completed");
        assert_eq!(credential_output_advisory(&payload), None);
    }
