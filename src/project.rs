...
pub(crate) fn query(endpoint: &str, sparql: &str) -> Result<String> {
    ...
    // NOT STARTED rather than started-and-abandoned. A request we do not send
    // cannot become a read quipu holds for a caller that is gone.
    let Some(timeout) = call_timeout() else {
        return Err(Error::Projection(format!(
            "projection budget spent before POST {url} — not started (aegis-h9c0no)"
        )));
    };
